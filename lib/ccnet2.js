...
CCNET.prototype = {
    ...
    execute: function(command, data, cb){
        ...
        if(this.busy){
            ...
            self.busyTimeOut[command]= setTimeout(function () {
             //   alert('call execute ' + command)
                self.execute(command, data, cb);
            }, 50)

            return cb(new Error("Device is busy"));
        }

        clearInterval(this.busyTimeOut[command]);
